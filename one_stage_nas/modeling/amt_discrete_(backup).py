...
class AMTDiscrete(nn.Module):
    def __init__(self, cfg):
        ...
        if os.path.exists(geno_file):
            ...
            if "hnasnet" in geno_file:
                # load original AutoDeepLab config
                geno_cell = [
                    ('dil_conv_5x5', 1),
                    ('sep_conv_3x3', 0),
                    ('sep_conv_3x3', 1),
                ...
                geno_path = [0, 0, 0, 1, 2, 1, 2, 2, 3, 3, 2, 1]
                genotype = (geno_cell, geno_path)
            else:
        ...
        self.stem2 = conv3x3_bn(64, 128, 1, relu=False)
        self.reduce = conv3x3_bn(128, self.f*self.num_blocks, 1, affine=False, relu=False)
